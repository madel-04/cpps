...
// constructor protegido (para uso de clases derivadas)
ClapTrap::ClapTrap(const std::string& name, int hp, int energy, int attack)
    : _name(name), _hitPoints(hp), _energyPoints(energy), _attackDamage(attack)
{
    std::cout << "ClapTrap " << _name << " constructed (custom stats)." << std::endl;
}
